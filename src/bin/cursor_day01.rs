    ...
    for rotation in inputs {
        ...
        match direction {
            'L' => {
                ...
                // for k in [1, distance]. This means start - k = 100*n for some integer n,
                // so k = start - 100*n. We need: 1 <= start - 100*n <= distance
                // Rearranging: start - distance <= 100*n <= start - 1
            ...
            'R' => {
                // Count zeros during rotation: we pass through 0 when (start + k) % 100 == 0
                // for k in [1, distance]. This means start + k = 100*n, so k = 100*n - start
